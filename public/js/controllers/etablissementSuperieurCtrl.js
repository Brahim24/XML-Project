app.controller('etablissementSuperieurCtrl',  function($rootScope, $scope, $http, $uibModal, $q, $log, uiGmapIsReady) {
	...
	$scope.all_collumns = [ 
		...
		{ name : 'REG', label: 'Region', ticked : true },
		{ name : 'PPRO', label: 'Description', ticked : false }, 
		{ name : 'DPRO', label: 'Annee', ticked : false }, 
		{ name : 'SCLE', label: 'Siecle', ticked : false }, 
	...
	$scope.getData = function() {
		...
		$http.get(url+query).then(function(response) {
			...
			$scope.listeEtablissement = response.data.data;
    ...
    $scope.buildMarkers = function(){
		...
		for(var index in $scope.listeEtablissement) {
			var element = $scope.listeEtablissement[index];
